...
export class Game extends Scene {
  ...
  create() {
    ...
    this.titleText = this.add.text(this.scale.width / 2, 30, 'I 100 Passi del Necromante', {
        fontSize: '26px', color: '#e94560', fontFamily: 'Arial, sans-serif', fontStyle: 'bold'
    }).setOrigin(0.5).setShadow(2, 2, '#000000', 2, true, true);

    this.minionsText = this.add.text(this.scale.width / 2, 65, 'Caricamento Armata da Reddit...', {
    ...
    this.fightText = this.add.text(this.scale.width / 2, this.scale.height - 80, 'AVVIA BATTAGLIA', {
  ...
  async fetchMinions() {
      try {
          ...
          const armySizeLimit = 10 + (state.upgrades.armySize * 2);

          const minions = await trpc.getMinions.query();
          this.minionsText.setText(`Armata generata: ${minions.length} Minions da Reddit`);
          
          const isBossBattle = this.registry.get('isBossBattle') || false;
          const stepsTaken = this.registry.get('currentNodeIndex') || 0;
          this.setupArmies(minions, armySizeLimit, state.depth, stepsTaken, isBossBattle);
      } catch (err) {
          console.error("Errore reddit:", err);
          this.minionsText.setText('Errore caricamento armata.');
      }
  }
  ...
  setupArmies(playerData: Minion[], armySizeLimit: number, depth: number, stepsTaken: number, isBossBattle: boolean) {
      // Pulisci vecchie armate se presenti
      ...
      if (isBossBattle) {
          ...
          for (let i = 0; i < enemyCount; i++) {
              ...
              let type: 'SKELETON' | 'GOLEM' | 'MAGE' = 'SKELETON';
  ...
  executeAttackCycle() {
      ...
      if (this.playerArmy.length === 0) {
          this.endBattle('Hai Perso l\'Armata...');
          return;
      }
      if (this.enemyArmy.length === 0) {
          this.endBattle('VITTORIA!');
          return;
      }
      ...
      const playerMages = this.playerArmy.filter((u, idx) => idx > 0 && u.data.type === 'MAGE');
      ...
      this.tweens.add({
          ...
          onYoyo: () => {
              ...
              if (enemyFront.data.hasBoneArmor) {
                  ...
                  this.showFloatingText(enemyFront.container.x, enemyFront.container.y - 40, 'SCUDO!', '#ffffff');
              } else {
  ...
  enemyCounterAttack(enemyFront: RenderedUnit, playerFront: RenderedUnit) {
      const enemyMages = this.enemyArmy.filter((u, idx) => idx > 0 && u.data.type === 'MAGE');
      ...
      this.tweens.add({
          ...
          onYoyo: () => {
              ...
              if (playerFront.data.hasBoneArmor) {
                  ...
                  if (shield) shield.destroy();
                  this.showFloatingText(playerFront.container.x, playerFront.container.y - 40, 'SCUDO!', '#ffffff');
              } else {
  ...
  endBattle(message: string) {
    if (this.soulsEarned > 0) {
        ...
        message += `\n+${this.soulsEarned} Anime`;
    }

    if (message.includes('VITTORIA')) {
    ...
    this.add.text(this.scale.width / 2, this.scale.height - 150, message, {
        fontSize: '48px',
        color: message.includes('VITTORIA') ? '#4caf50' : '#f44336',
        fontStyle: 'bold',
